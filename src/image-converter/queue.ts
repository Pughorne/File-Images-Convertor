...
const createImagesQueue = (outputDir: string) => {
  ...
  const runConverter = () => {
    setTimeout(async () => {
      ...
      for (const imageID of imagesIDs) {
        ...
        convertedImages.set(imageID, {status: imageStatus, filepath: image.outputFilepath!});
  ...
  return ({
    add: (image: InputImage) => {
      ...
      image.outputFilepath = newImagePath(image.filepath, outputDir, image.options.targetFormat);
      unconvertedImages.set(imageID, image);
      return imageID;
    },
    get: (imageID: string): OutputImage => {
      const outputImage = convertedImages.get(imageID) || {status: ImageStatus.NOT_FOUND} as OutputImage;
